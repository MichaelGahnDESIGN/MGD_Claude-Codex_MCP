...
Für das konkret entstandene Projekt im `WORKSPACE/` soll zusätzlich eine eigene `Projekt Dokumentation` in `WORKSPACE/DOKUMENTATION/` geführt werden.

## Root-Regel

Direkt im Root von `PROJEKT/DOKUMENTATION/` sollen nur diese Dateien liegen:
...
- nach Änderungen an Ordnern, Agenten oder Skills den lokalen Generator für die Dokumentationsdaten erneut ausführen
  - Standardbefehl: `python3 PROJEKT/DOKUMENTATION/Dokumentation-Skills/generate_dokumentationsdaten.py`
- Dokumentationswerkzeuge gehören in `PROJEKT/DOKUMENTATION/` und nicht in `WORKSPACE/`
...
- den Aufbau von `PROJEKT/DOKUMENTATION/` erklären
- zusätzlich die getrennte `Projekt Dokumentation` in `WORKSPACE/DOKUMENTATION/` zugänglich machen
- eine klare Ordnerübersicht für Root, `AI/` und `PROJEKT/` enthalten
